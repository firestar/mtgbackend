...
class Route{
  ...
  constructor(){
    if(!Route.path) {
      Route.path = new this.RegisterDecoratorClass();
    }
    if(!Route.registry) {
      Route.registry = new this.RouteRegistryClass();
    }
  }
  RegisterDecoratorClass = class {
  ...
  RouteRegistryClass = class {
    ResponseData = class ResponseData {
      publicData;
      privateData;
    ...
    PathElement = class PathElement {
      paths;
      path:string;
      rootFunction=null;
      CurrentExecution = class CurrentExecution{
        client;
        path_level;
        currentLevel;
      ...
      FunctionExecution = class FunctionExecution{
        next=null;
        method=null;
        matches=null;
        path=null;
      ...
      addFunction(method, matches, path){
        if(this.rootFunction){
          this.rootFunction.add(new this.FunctionExecution(method, matches, path));
        }else{
          this.rootFunction=new this.FunctionExecution(method, matches, path);
        }
      }
      call(client, path_level, log){
        ...
        new this.CurrentExecution(client, path_level, this, this.rootFunction, log).exec();
      }
    ...
    constructor() {
      this.root = new this.PathElement("/");
    ...
    add(full_path, link_method) {
      ...
      var currentPath: self.PathElement = this.root;
      var matches = {};
      if (path_parts[0] != '') {
        path_parts.forEach((path_string, index) => {
          ...
          if (!currentPath.paths[path_string]) {
            currentPath.paths[path_string] = new self.PathElement(path_string);
          }
    ...
    call(full_path: string, req, res, data = {}) {
      var self = this;
      var jsonString = '';
      ...
      req.on('end', function () {
        ...
        var response: self.ResponseData = new self.ResponseData(data);
        path_parts = path_parts.slice(1, path_parts.length);
        var currentPath: self.PathElement = self.root;
        var client = new self.Client(path_parts, req, res, response, query, req.method, pageData, execution_info);
        async.parallel(
          [
            () => {
